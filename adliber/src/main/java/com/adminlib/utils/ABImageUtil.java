...
public class ABImageUtil {
     ...
     * @param desiredWidth  需要图片的宽度
     * @param desiredHeight 需要图片的高度
     * @return the bitmap
     */
     ...
     * 获取缩小的比例.
     *
     * @param srcWidth      图片的固有的宽度
     * @param srcHeight     图片固有的高度
     * @param desiredWidth  图片缩放后的宽度
     * @param desiredHeight 图片缩放后的高度
     * @return scale缩放的比例
     */
    ...
    private static boolean checkBitmap(Bitmap bitmap) {
        if (bitmap == null) {
            ABLogUtil.e("原图Bitmap为空了");
            return false;
        }

        if (bitmap.getWidth() <= 0 || bitmap.getHeight() <= 0) {
            ABLogUtil.e("原图Bitmap大小为0");
            return false;
        }
        return true;
    }
    ...
    private static boolean checkSize(int desiredWidth, int desiredHeight) {
        if (desiredWidth <= 0 || desiredHeight <= 0) {
            ABLogUtil.e("请求Bitmap的宽高参数必须大于0");
            return false;
        }
        return true;
    }
    ...
    public static TransitionDrawable drawableToTransitionDrawable(
        ...
        try {
            ...
            mBitmapDrawable = new TransitionDrawable(new Drawable[]{
                    new ColorDrawable(transparent), drawable});
        } catch (Exception e) {
            e.printStackTrace();
        }
        return mBitmapDrawable;
        ...
        if (bitmap != null) {
            try {
                if (!bitmap.isRecycled()) {
                    ABLogUtil.d("Bitmap释放" + bitmap.toString());
                    bitmap.recycle();
                }
            } catch (Exception e) {
            }
     ...
     * @param reqWidth  the reqWidth
     * @param reqHeight the reqHeight
     * @return inSampleSize
     */
     ...
     *
     * @param filePath 为减低至
     * @param targetW  需要将图片压缩到的宽度
     * @param targetH  需要将图片压缩到的高度
     * @return the Bitmap
     */
